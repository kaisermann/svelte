			...
			this.slots.size > 0
				? [
					...
					${Array.from(this.slots).map(([name, slot]) => {
						return p`${name}: [${slot.block.name}, ${slot.get_context ||
							null}, ${slot.get_changes || null}]`;
					})}
				}`,
						p`$$scope: {
					ctx: #ctx
			...
			if (uses_spread) {
				...
				this.node.attributes.forEach((attr, i) => {
					...
					if (attr.is_spread) {
						...
						changes.push(
							condition ? x`${condition} && ${value_object}` : value_object
						);
					} else {
						...
						changes.push(
							condition ? x`${condition} && ${obj}` : x`${levels}[${i}]`
						);
					}
				});
